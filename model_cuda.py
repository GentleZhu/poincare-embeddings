...
class Embedding(nn.Module):
    def __init__(self, size, dim, dist=PoincareDistance, max_norm=1, is_cuda=True):
    ...
    def init_weights(self, scale=1e-4):
        if self.is_cuda:
            self.lt.state_dict()['weight'].uniform_(-scale, scale).cuda()
        else:
            self.lt. state_dict()['weight'].uniform_(-scale, scale)

    def forward(self, inputs):
        if self.is_cuda:
            e = self.lt(inputs).cuda()
        else:
            e = self.lt(inputs)
...
class SNEmbedding(Embedding):
    ...
    def loss(self, preds, targets, weight=None, size_average=True):
        ...
        return lossfn(preds, targets.cuda())
